<%--
  Created by IntelliJ IDEA.
  User: Yaros
  Date: 05.05.2023
  Time: 9:05
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ include file="./header.jsp" %>
<html>
<head>
    <title>Trainers</title>
</head>
<body>

<section class="page-header bg_img" data-background="/static/images/banner/05.jpg">
    <div class="container">
        <h3 class="title">
            <span class="shape-wrapper">
                <span class="shape"></span>Наші тренери<span class="shape"></span></span>
        </h3>
    </div>
</section>

<div class="breadcrumb-section">
    <div class="container">
        <div class="breadcrumb-wrapper">
            <div class="breadcrumb-title"><h6 class="title">тренери</h6></div>
            <ul class="breadcrumb">
                <li><a href="<c:url value="./home"/>">Дім</a></li>
                <li><a href="/trainers">Тренери</a></li>
                <li>Андрій Матвієнко</li>
            </ul>
        </div>
    </div>
</div>

<div class="container">
    <div class="trainer-view">
        <div class="row">
            <div class="col-lg-6 traning-info">
                <h1 class="trainer-name">
                    Андрій Матвієнко           </h1>
                <div class="trainer-description">
                    У фітнесі вже 15 років: пройшов шлях від бодібілдингу до функціональних тренувань та спортивних змагань. Сертифікований тренер та учасник змагань з гирьового спорту та пауерліфтингу.

                    Андрій цінує свою роботу за можливість допомагати людям. Переконаний, що від тебе залежить перший крок, треба лише зробити його у напрямку клубу!

                    Веде такі тренування, як Games, Animal Flow, Power, Stretching, Recovery, Climb.
                </div>
            </div>
            <div class="col-lg-6 trainer-image top-trainer">
                <img class="img-responsive" src="/static/images/trainers/trainer6.jpg" alt="">        </div>
        </div>
        <div class="row">
            <div class="col-lg-12">
                <a class="btn btn-block btn-warning main-training-btn" href="/gym/schedule" onclick="dataLayer.push({'event': 'smartass', 'eventCategory': 'userclicks', 'eventAction': 'button', 'eventLabel': 'applic'});">
                    Записатись                </a>
            </div>
        </div>
    </div>    </div>
</body>
<%@ include file="./footer.jsp" %>
</html>
